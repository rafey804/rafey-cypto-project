// @ts-check
/**
 * Pure-JS local mock data for Vercel Edge Runtime (api/bootstrap.js).
 * Mirrors server/_shared/local-mock-data.ts but as a plain .js file so the
 * Edge bundler can resolve it without crossing the TypeScript boundary.
 */

export function getLocalMockDataForKey(key) {
  const now = Date.now();
  ...
  if (key.includes('market:gold-extended:v1')) {
    return {
      extended: {
        technicals: { rsi14: 64.2 + (now % 10) / 10, macd: 'Bullish Crossover', support: 4020, resistance: 4100, trend: 'Strong Bullish' },
  ...
  // Generic rich fallback for any other key
  return {
    available: true,
    unavailable: false,
    dataAvailable: true,
    ...
    quotes: [
      ...
      { symbol: 'GC=F', name: 'Gold Spot XAUUSD', price: 4063.78 + jitter1, change: 2.09, changePercent: 0.05, volume: 154200, lastUpdate: now }
    ],
